            ...
            @foreach ($fields as $fieldKey)
              ...
              <div>
                ...
                @if ($meta?->type === 'select' && is_array($meta?->options))
                  <select class="mt-1 w-full rounded-lg border border-slate-300 px-3 py-2" id="{{ $fieldKey }}" name="{{ $fieldKey }}" required>
                    @foreach ($meta->options as $value => $text)
                      <option value="{{ $value }}">{{ $text }}</option>
                    @endforeach
                  </select>
                @else
                  <input class="mt-1 w-full rounded-lg border border-slate-300 px-3 py-2" id="{{ $fieldKey }}" name="{{ $fieldKey }}" type="{{ $meta?->type ?? 'text' }}" required />
                @endif
              </div>
            @endforeach
          </div>
