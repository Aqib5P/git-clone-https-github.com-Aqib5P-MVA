...
<html lang="en">
  <head>
    ...
    <title>Validate IP</title>
    <script src="https://cdn.tailwindcss.com"></script>
  </head>
  <body class="bg-slate-50 text-slate-900">
    <div class="max-w-md mx-auto mt-24 bg-white border border-slate-200 rounded-2xl p-6 shadow-sm">
      <h3 class="text-xl font-semibold mb-2">Validate Office IP</h3>
      <p class="text-sm text-slate-600 mb-4">Enter admin credentials to allow this IP.</p>
      @if ($errors->any())
        <div class="mb-4 rounded-lg border border-red-200 bg-red-50 text-red-700 px-3 py-2">
          {{ $errors->first() }}
        </div>
      @endif
      <form method="post" action="{{ route('ip.validate.submit') }}" class="space-y-4">
        @csrf
        <div>
        ...
        <button type="submit" class="w-full rounded-lg bg-blue-600 text-white py-2 hover:bg-blue-700">Validate IP</button>
      </form>
    </div>
  </body>
</html>
