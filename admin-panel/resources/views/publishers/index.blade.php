...
@section('content')
  <div class="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm mb-6">
    <h3 class="text-lg font-semibold mb-4">Add Publisher</h3>
      ...
      <tbody>
        @forelse ($publishers as $publisher)
          <tr class="border-t border-slate-100">
            <td class="py-2">{{ $publisher->code }}</td>
            <td class="py-2">{{ $publisher->name }}</td>
            <td class="py-2">{{ $publisher->active ? 'Active' : 'Inactive' }}</td>
            <td class="py-2">
              <form method="post" action="{{ route('publishers.toggle', $publisher) }}" class="inline">
                @csrf
                <button type="submit" class="border border-slate-300 px-2 py-1 rounded-lg text-xs hover:bg-slate-100">{{ $publisher->active ? 'Disable' : 'Enable' }}</button>
              </form>
            </td>
          </tr>
        @empty
          <tr>
            <td colspan="4" class="text-slate-500 py-3">No publishers yet.</td>
          </tr>
        @endforelse
      </tbody>
    </table>
